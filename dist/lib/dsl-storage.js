...
export function createDslStorage(options = {}) {
    ...
    return Object.freeze({
        ...
        save(file) {
            ...
            if (typeof file.source !== 'string')
                throw new TypeError('DSL source must be a string');
...
export async function readDslFile(file, maxSourceBytes = 1024 * 1024) {
    if (!(file instanceof File))
        throw new TypeError('file must be a browser File');
    if (file.size > maxSourceBytes)
        throw new TypeError('DSL file exceeds the configured byte limit');
