...
const defaultIcons: Record<ApplicationMenuAction, string> = {
  open: '📂',
  reload: '↻',
  about: 'i',
  close: 'x'
...
function textFor(locales: Record<string, ApplicationMenuItemText>, locale: string): ApplicationMenuItemText {
  ...
  for (const key of ['open', 'reload', 'about', 'close'] as const) {
...
export function createApplicationMenu(options: ApplicationMenuOptions): ApplicationMenu {
  ...
  const callbacks: Record<ApplicationMenuAction, (() => unknown | Promise<unknown>) | undefined> = {
    open: options.onOpen,
    reload: options.onReload,
    about: options.onAbout,
    close: options.onClose
  };
  const positions: Record<ApplicationMenuAction, [string, string]> = {
    open: ['10%', '25.5556%'],
    reload: ['53.3333%', '25.5556%'],
    about: ['10%', '58.8889%'],
    close: ['53.3333%', '58.8889%']
  };
  const buttons = new Map<ApplicationMenuAction, HTMLButtonElement>();
  let locale = options.initialLocale ?? (locales.ja ? 'ja' : Object.keys(locales)[0] ?? 'en');
  let reloadEnabled = options.reloadEnabled ?? true;
  let disposed = false;

  for (const action of ['open', 'reload', 'about', 'close'] as const) {
  ...
  function show(nextLocale = locale): string {
  ...
  function setReloadEnabled(enabled: boolean): void {
    if (disposed) throw new TypeError('application menu is disposed');
